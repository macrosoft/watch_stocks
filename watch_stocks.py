...
def status_command(update, context):
	...
	if len(rows) > 0:
		...
		cursor.execute("SELECT code, fullname FROM ticker  WHERE value NOT NULL ORDER BY random() LIMIT 1")
		rows = cursor.fetchall()
		for row in rows:
...
while 1:
	...
	cursor.execute("UPDATE subscribe SET refval = (SELECT value FROM ticker WHERE id = subscribe.ticker) WHERE AND type = 1 AND refval > (SELECT value FROM ticker WHERE id = subscribe.ticker)")
	conn.commit()
